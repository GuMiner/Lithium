(() => {
  ...
  function convertNumbers() {
    ...
    for (var j2 = 2; j2 <= 36; j2++) {
      let convertedNumbers = "(" + j2 + ") ";
      if (j2 < 10) {
        convertedNumbers += " ";
      }
      for (var i2 = 0; i2 < parts.length; i2++) {
        let number = parseInt(parts[i2], base);
        convertedNumbers += number.toString(j2) + delimiter;
      }
  ...
  function convertSubstituteNumbers() {
    ...
    if (isChecked("rotateSubst")) {
      document.getElementById("rotateAmount").disabled = false;
      outputString += rot13(parts, rotateValue) + "\r\n";
    } else {
      document.getElementById("rotateAmount").disabled = true;
    }
    if (isChecked("ASCIIConvertSubst")) {
      outputString += numberToAscii(parts) + "\r\n";
    }
    if (isChecked("MorseConvertSubst")) {
      document.getElementById("MorseDot").disabled = false;
      document.getElementById("MorseDash").disabled = false;
      outputString += convertMorse(parts, morseDot, morseDash) + "\r\n";
    } else {
      document.getElementById("MorseDot").disabled = true;
      document.getElementById("MorseDash").disabled = true;
    }
    document.getElementById("convertedNumbersSubst").textContent = outputString;
  }
  function letterToNumber(parts) {
    let a1z26String = "";
    for (let i2 = 0; i2 < parts.length; i2++) {
      let integer = (parseInt(parts[i2]) - 1) % 26;
      if (isNaN(integer)) {
        a1z26String += parts[i2];
      } else {
  ...
  function rot13(parts, rotateAmount) {
    let rot13String = "";
    for (let i2 = 0; i2 < parts.length; i2++) {
      if (parts[i2].length == 1) {
        let charValue = parts[i2].toUpperCase().charCodeAt(0);
        ...
        rot13String += parts[i2];
      }
      rot13String += " ";
    }
    return rot13String;
  }
  function numberToAscii(parts) {
    let asciiString = "";
    for (var i2 = 0; i2 < parts.length; i2++) {
      let integer = parseInt(parts[i2]);
      if (isNaN(integer)) {
        asciiString += parts[i2];
      } else {
  ...
  function convertMorse(parts, dotChar, dashChar) {
    let morseString = "";
    for (var i2 = 0; i2 < parts.length; i2++) {
      let letter = parseMorseCharacter(parts[i2], dotChar, dashChar);
      morseString += letter;
    }
    return morseString;
  }
  ...
  function parseMorseCharacter(character, dotChar, dashChar) {
  ...
  window.convertNumbers = convertNumbers;
  window.convertSubstituteNumbers = convertSubstituteNumbers;
})();
