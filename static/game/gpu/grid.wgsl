...
@group(0) @binding(0) var<storage,read_write> gridData : array<u32, simWidth*simHeight>;
...
fn fragmentMain( @builtin(position) position: vec4f) -> @location(0) vec4f {
    ...
    let gridDataIndex = vec2u((position.xy / viewport) * vec2f(f32(simWidth), f32(simHeight)));
    ...
    let colorIndex = gridData[dataIndex];
